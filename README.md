...
| IA como servicio — API-Keys (`POST/GET /me/api-keys`, `DELETE /me/api-keys/{id}`, `GET /me/api-keys/{id}/usage`) | **Real, contra el gateway de IA directamente — no pasa por `idempotencia-back`.** Se descartó el patrón BFF: el gateway (FastAPI sobre Ollama, compatible con el SDK de OpenAI) valida el JWT de la plataforma por su cuenta y expone `/me/api-keys` — mismo `Authorization: Bearer` de siempre, otra base URL (`VITE_AI_GATEWAY_BASE_URL`, ahora imprescindible: sin ella no solo fallan los ejemplos, falla el servicio entero). Dos gotchas del contrato del gateway, resueltas en `shared/api/aiApi.ts`: (1) las fechas llegan en UTC **sin sufijo `Z`** — `shared/lib/parseUtc.ts` las normaliza al mapear la respuesta, antes de que le lleguen a la UI; (2) los errores vienen en formato OpenAI (`{ error: { code, message } }`) y hay que ramificar por `code` (tipo cerrado, `AiKeyErrorCode`), nunca por `message` — `useAiKeyErrorHandler` centraliza esa traducción, incluyendo que un `401 invalid_token` es la sesión de la plataforma vencida (cierra sesión y redirige a login), no una API-Key inválida. Una clave puede estar `isActive: true` y ya no servir si `expiresAt` pasó — `computeAiKeyStatus` calcula el estado real (`Activa`/`Expirada`/`Revocada`) combinando ambos campos, el badge no confía solo en `isActive`. `apiKey` completa solo se ve una vez, igual que la contraseña de una BD — se muestra vía el mismo `CredentialRevealCard` genérico, nunca se persiste en `localStorage`. El formulario de creación por ahora solo pide `name`: el gateway ignora a propósito cualquier límite de tasa/tokens que se mande, los fija el operador. El bloque de ejemplos de uso (`features/manage-ai-keys/ui/AiApiExamples.tsx`) usa el `base_url`/`model` que devuelve la respuesta de creación (no una constante propia) — si el despliegue cambia de host o de modelo, el ejemplo no queda desactualizado. |
| DNS — subdominios propios (`GET /dns/zone`, `POST/GET /dns`, `PUT/DELETE /dns/{id}`) | **Real**, contra `idempotencia-back` (mismo `VITE_API_BASE_URL`, mismo JWT que bases de datos — no tiene nada que ver con el gateway de IA). El dominio y el patrón (`{label}.idempotencia.coderhivex.com`) salen siempre de `GET /dns/zone`, nunca hardcodeados: cambian entre ambientes. Validación de cliente solo para forma (3–63 caracteres, sin puntos, IPv4 bien formada) — a propósito **no** se duplican en el front: nombres reservados, rangos de IP privada ni chequeo de disponibilidad, esas listas viven en el servidor y se desincronizan si se copian. Cuota de 5 por usuario, con el botón de crear deshabilitado al llegar al tope. Eliminar pide escribir el nombre completo para confirmar (`ConfirmDialog` con el nuevo modo opcional `confirmationText`) — no hay paso de desactivación previo como en bases de datos, el nombre queda libre de inmediato. El 429 deshabilita el botón de crear con una cuenta regresiva real (`ApiError.retryAfter`, agregado a `httpClient.ts`), y un 409 (nombre tomado) sugiere una variante (`{label}-2`) sin pretender adivinar disponibilidad. |
...
| `VITE_AI_GATEWAY_BASE_URL` | Base del gateway de IA — **imprescindible**, ver [Integración con backend](#integración-con-backend). Valor real solo en `.env.local` (no versionado). Sin definir, todo el servicio de IA queda roto (no solo los ejemplos de código). |
...
**Importante:** `docker-compose.yml` no trae valores por defecto para `FRONTEND_PORT`, `FRONTEND_IMAGE`, `FRONTEND_CONTAINER_NAME` ni `VITE_API_BASE_URL` — si se corre `docker compose up --build` sin `--env-file` y sin definirlas en el entorno, falla (`no port specified` como mínimo). Siempre usar `--env-file .env.local` (o exportarlas todas antes) para levantarlo. `VITE_AI_GATEWAY_BASE_URL` técnicamente sí tiene default (vacío) y el build no falla sin ella, pero **no es opcional en la práctica**: sin definirla, todo el servicio de IA queda roto (no solo los ejemplos de código, como cuando era decorativa). Es fácil que este build-arg se quede afuera silenciosamente porque no rompe el build — pasó una vez en QA — así que conviene revisar explícitamente que esté seteada en el `.env` del entorno donde se despliega.
...
- **Variables de entorno:** `VITE_API_BASE_URL` y `VITE_AI_GATEWAY_BASE_URL` (ver [Integración con backend](#integración-con-backend)). Ambas se inyectan en **build time** vía `--build-arg`:
  ```bash
  docker build \
    --build-arg VITE_API_BASE_URL=https://api.idempotencia.andrescortes.dev \
    --build-arg VITE_AI_GATEWAY_BASE_URL=<url-real-del-gateway-de-ia> \
    -t idempotencia-frontend .
  ```
  `VITE_API_BASE_URL` trae un valor por defecto si no se pasa el `--build-arg`; `VITE_AI_GATEWAY_BASE_URL` no — queda vacía si se omite, y a diferencia de antes eso ya no es solo cosmético: rompe el servicio de IA completo, no solo los ejemplos de código (ver [Integración con backend](#integración-con-backend)). Ninguna se puede cambiar en runtime, porque el resultado ya es HTML/JS estático servido por Nginx.
...
5. **Variables de entorno:** `VITE_API_BASE_URL` y `VITE_AI_GATEWAY_BASE_URL` — ver la nota de build-time en la sección de Docker.
...
│                 # manage-database, manage-ai-keys, manage-dns, logout
├── entities/     # Modelos de negocio + su UI: user (sesión), database, ai-key, dns-record, platform-stats
├── shared/       # Sin lógica de negocio, reutilizable en todo el proyecto
│   ├── api/       # authApi/databaseApi/aiApi/dnsApi/platformStatsApi + httpClient (idempotencia-back) +
│   │              # aiGatewayClient (gateway de IA, backend aparte) + session-storage + mocks
│   ├── config/    # Design tokens (colores, tipografía, espaciado, motion) de marca — claro y oscuro
│   ├── lib/       # Helpers puros (parseUtc, formatDate, formatRelativeDate…) + theme/ (ThemeProvider) +
│   │              # motion/ (constantes de Framer Motion)
│   └── ui/        # Componentes de UI genéricos (Button, Input, CredentialRevealCard, ComingSoonPanel,
│                  # ConfirmDialog —con modo opcional "escribí X para confirmar"—, Toast, ThemeToggle,
│                  # PageTransition, íconos)
