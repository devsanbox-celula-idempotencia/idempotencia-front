...
| Reactivar una BD desactivada | **UI lista, sin backend todavía.** El botón "Reactivar base de datos" ya existe en `DatabaseDetailPanel` (visible cuando `status === 'Inactive'`, junto a "Eliminar") con su propio `ConfirmDialog`, pero `useManageDatabase.ts` lanza un error explicando que el backend aún no comparte el endpoint — no hay ninguna URL inventada. Cuando llegue la ruta real, se agrega `reactivateDatabase` a `databaseApi.ts` y se reemplaza ese `throw` por la llamada real; no hace falta tocar la UI. |
